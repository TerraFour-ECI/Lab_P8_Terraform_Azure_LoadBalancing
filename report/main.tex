...
The component diagram lives in \texttt{docs/DIAGRAMS.md} and uses
advanced Mermaid notation (subgraphs, classed nodes, custom theming).
It captures the four logical layers: identity (Azure AD + OIDC),
remote state (Storage Account + container + blob lease), workload
(Resource Group containing VNet, NSG, NICs, VMs and the LB) and the
external actors (developer CLI, GitHub runner and the end user).

\begin{figure}[H]
\centering
\imgplaceholder{6cm}{media/component-diagram.png}
\caption{Component diagram of the Lab \#8 deployment, including identity
federation and the remote state backend.}
\label{fig:component}
\end{figure}

\subsection{Sequence diagram}

The sequence diagram depicts what happens when an end user issues
\texttt{curl http://<lb\_public\_ip>}: name resolution, TCP handshake
intercepted by the LB, 5-tuple hashing that selects a backend NIC, NSG
acceptance of the inbound \texttt{80/TCP} flow, and the response
returning back through the same path. A second sequence diagram in the
same document covers the CI/CD flow with OIDC.

\begin{figure}[H]
\centering
\imgplaceholder{6cm}{media/sequence-diagram.png}
\caption{Request flow through the Public IP, the Load Balancer hash and
the NSG before the nginx VM responds with the page that embeds the
hostname.}
\label{fig:sequence}
\end{figure}
...
\begin{figure}[H]
\centering
\imgplaceholder{4cm}{media/curl-roundrobin.png}
\caption{Successive curl requests rotating between \texttt{lab8-vm-0}
and \texttt{lab8-vm-1}, confirming the LB hash distribution.}
